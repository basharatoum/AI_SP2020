...
def extractSymbols(exps,m):
    s = []
    if exps.symbol[0] !='' and not (exps.symbol[0] in m):
        s.append(exps.symbol[0])
    else:
        for e in exps.subexpressions:
            s += extractSymbols(e,m)
    return s


def TTEntail(KB,Alpha,m):
    s = []
    s = extractSymbols(KB,m)
    s += extractSymbols(Alpha,m)
    s = list(set(s))
    return TTCheck(KB,Alpha,s,m)

def PLTrue(S,model):
    if S.symbol !=['']:
        if S.symbol[0] in model:    
            return model[S.symbol[0]]
        else: 
            return False
    
    elif S.connective ==['and']:
        for e in S.subexpressions:
            if (PLTrue(e,model)==False):
                return False
        return True
    elif S.connective ==['or']:
        for e in S.subexpressions:
            if(PLTrue(e,model)==True):
                return True
        return False
    elif S.connective==['if']:
        if PLTrue(S.subexpressions[0],model)==True and PLTrue(S.subexpressions[1],model)==False:
            return False
        return True
    elif S.connective==['iff']:
        if PLTrue(S.subexpressions[0],model) != PLTrue(S.subexpressions[1],model):
            return False
        return True
    elif S.connective==['not']:
        return not PLTrue(S.subexpressions[0],model)
    elif S.connective==['xor']:
        c = 0
        for e in S.subexpressions:
            if(PLTrue(e,model)==True):
                c+=1
...
def TTCheck(KB,Alpha,Sym,model):
    if not Sym:
        if PLTrue(KB,model):
        ...
        m1[F] = True
        m2[F] = False
        return TTCheck(KB,Alpha,R,m1) and TTCheck(KB,Alpha,R,m2)

def check_true_false(KB,Alpha,notAlpha,m):

    A = TTEntail(KB,Alpha,m)
    nA =  TTEntail(KB,notAlpha,m)
    if A and not nA:
        return "definitely true."
    elif nA and not A:
        return "definitely false."
    elif not nA and not A:
